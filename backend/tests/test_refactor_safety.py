...
def test_no_llm_returns_failure_gracefully(refactor_agent_no_llm, long_method_code):
...
def test_no_llm_all_smells_no_crash(refactor_agent_no_llm, clean_code):
...
def test_no_llm_original_code_preserved(refactor_agent_no_llm, long_method_code):
    result = refactor_agent_no_llm.refactor(long_method_code, "long_method")
    assert result["original_code"] == long_method_code
    assert result["refactored_code"] == long_method_code

def test_no_llm_has_strategy_field(refactor_agent_no_llm, clean_code):
...
def test_syntax_preserved_on_good_llm_output(long_method_code):
...
def test_rollback_on_invalid_llm_output(long_method_code):
...
def test_rollback_keeps_original_parseable(long_method_code):
...
def test_idempotency_no_llm(refactor_agent_no_llm, clean_code):
...
def test_idempotency_mock_llm(long_method_code):
...
def test_extract_code_fallback_plain():
    from refactor_agent.refactor_agent import RefactorAgent
    agent = RefactorAgent.__new__(RefactorAgent)
    agent._llm = None
    raw = "def plain(): pass"
...
def test_llm_exception_triggers_graceful_failure(long_method_code):
