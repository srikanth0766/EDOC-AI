...
def test_lambda_zero_stable_history(sprint_risk_model):
...
def test_lambda_zero_single_sprint_no_error(sprint_risk_model):
...
def test_mu_zero_growing_trend(sprint_risk_model):
...
def test_rapid_growth_high_risk(sprint_risk_model):
...
def test_rapid_growth_recommendation_critical(sprint_risk_model):
...
def test_negative_growth_improving_trend(sprint_risk_model):
...
def test_negative_growth_low_risk(sprint_risk_model):
...
def test_risk_probability_always_in_range(sprint_risk_model):
...
def test_no_division_by_zero(sprint_risk_model):
    """Single-element history must not crash."""
    result = sprint_risk_model.predict([5])
    assert result is not None
    assert 0.0 <= result["risk_probability"] <= 1.0

def test_identical_history_no_divergence(sprint_risk_model):
...
def test_zero_smells_throughout(sprint_risk_model):
...
def test_historical_replay_mae_threshold(sprint_risk_model):
    """
...
def test_refactor_history_reduces_risk(sprint_risk_model):
...
def test_recommendation_is_string(sprint_risk_model):
    result = sprint_risk_model.predict([3, 4, 6])
    assert isinstance(result["recommendation"], str)
    assert len(result["recommendation"]) > 0

def test_above_threshold_trend_when_current_exceeds(sprint_risk_model):
