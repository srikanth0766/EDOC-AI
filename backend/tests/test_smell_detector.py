...
class TestSmellDetector:
    ...
    def test_clean_code_no_smells(self, smell_detector, clean_code):
        smells = smell_detector.detect(clean_code)
        assert smells == [], f"Expected no smells, got: {[s.smell for s in smells]}"

    def test_detect_to_dict_clean(self, smell_detector, clean_code):
    ...
    def test_detects_long_method(self, smell_detector, long_method_code):
    ...
    def test_long_method_confidence_over_half(self, smell_detector, long_method_code):
        smells = smell_detector.detect(long_method_code)
        lm = next(s for s in smells if s.smell == "long_method")
        assert lm.confidence > 0.5, f"Expected confidence > 0.5, got {lm.confidence}"

    def test_long_method_has_refactor_hint(self, smell_detector, long_method_code):
        smells = smell_detector.detect(long_method_code)
        lm = next(s for s in smells if s.smell == "long_method")
    ...
    def test_detects_god_class_by_method_count(self, smell_detector, god_class_code):
        smells = smell_detector.detect(god_class_code)
        names = [s.smell for s in smells]
        assert "god_class" in names, f"god_class not detected in {names}"

    def test_god_class_confidence_positive(self, smell_detector, god_class_code):
        smells = smell_detector.detect(god_class_code)
        gc = next(s for s in smells if s.smell == "god_class")
        assert gc.confidence > 0.0

    def test_small_class_no_god_class(self, smell_detector):
    ...
    def test_detects_deep_nesting(self, smell_detector, deep_nesting_code):
        smells = smell_detector.detect(deep_nesting_code)
        names = [s.smell for s in smells]
        assert "deep_nesting" in names, f"deep_nesting not found in {names}"

    def test_shallow_nesting_no_smell(self, smell_detector):
        code = "def f(a):\n    if a:\n        return 1\n    return 0\n"
        smells = smell_detector.detect(code)
        names = [s.smell for s in smells]
    ...
    def test_detects_high_complexity(self, smell_detector, high_complexity_code):
        smells = smell_detector.detect(high_complexity_code)
        names = [s.smell for s in smells]
        assert "high_complexity" in names, f"high_complexity not found in {names}"

    def test_simple_function_no_high_complexity(self, smell_detector):
        code = "def f():\n    return 42\n"
        smells = smell_detector.detect(code)
        names = [s.smell for s in smells]
    ...
    def test_detects_large_parameter_list(self, smell_detector, large_param_code):
        smells = smell_detector.detect(large_param_code)
        names = [s.smell for s in smells]
        assert "large_parameter_list" in names

    def test_few_params_no_smell(self, smell_detector):
    ...
    def test_detects_feature_envy(self, smell_detector):
        code = (
            "import os\n"
            "import sys\n"
            "import json\n"
    ...
    def test_results_sorted_by_confidence_desc(self, smell_detector, long_method_code):
        smells = smell_detector.detect(long_method_code)
        if len(smells) >= 2:
            confidences = [s.confidence for s in smells]
            assert confidences == sorted(confidences, reverse=True)

    def test_confidence_in_range(self, smell_detector, long_method_code):
        smells = smell_detector.detect(long_method_code)
        for s in smells:
            assert 0.0 <= s.confidence <= 1.0, f"Confidence out of range: {s.confidence}"

    def test_severity_valid_values(self, smell_detector, long_method_code):
        smells = smell_detector.detect(long_method_code)
        for s in smells:
            assert s.severity in ("error", "warning", "info")

    def test_detect_to_dict_has_required_keys(self, smell_detector, long_method_code):
    ...
    def test_line_numbers_valid(self, smell_detector, long_method_code):
        smells = smell_detector.detect(long_method_code)
        for s in smells:
            assert s.start_line >= 1
            assert s.end_line >= s.start_line

    def test_invalid_code_returns_empty(self, smell_detector):
        smells = smell_detector.detect("def broken(:\n    pass")
        assert smells == []

    def test_empty_code_returns_empty(self, smell_detector):
